...
class PreONNX(object):
    ...
    def reason_all2(self, image_1, image_2_list):
        result = []
...
class Predict(object):
    def __init__(
            self, per_path="siamese.onnx", yolo_path="yolo.onnx", provider="CPUExecutionProvider"
    ):
        self.yolo = YOLOV5_ONNX(
            yolo_path, classes=["target", "title", "char"], providers=[provider]
        )
        self.pre = PreONNX(per_path, providers=[provider])
    ...
    def run(self, image_path):
        ...
        for m, img_char in enumerate(chars):
            if len(targets) == 0:
                ...
                slys = self.pre.reason_all2(img_char, img_target_list)
    ...
    def run2(self, target_image_path, que_path_list):
        img = self.open_image(target_image_path)
        ques = [self.open_image(que_path) for que_path in que_path_list]
        data = self.yolo.decect(target_image_path)
        # 需要选择的字
        targets = [i.get("crop") for i in data if i.get("classes") == "target"]
        # 根据坐标进行排序
        result = []
        for m, img_char in enumerate(ques):
            if len(targets) == 0:
                break
            elif len(targets) == 1:
                slys_index = 0
