...
def get_icon_position(captcha_img_data, icon_img_list: list[Image.Image], pro=False):
...
def predict_captcha(img_input: Image.Image, icon_input_1, icon_input_2, icon_input_3):
    ...
    try:
        ...
        positions = get_icon_position(img_input, [icon_input_1, icon_input_2, icon_input_3], False)
...
def predict_captcha_pro(img_input: Image.Image, icon_input_1, icon_input_2, icon_input_3):
    t = time.time()
    try:
        if None in [img_input, icon_input_1, icon_input_2, icon_input_3]:
            raise Exception("Error! 未上传图片!")
        positions = get_icon_position(img_input, [icon_input_1, icon_input_2, icon_input_3], True)
        img = draw(img_input, positions)
    except Exception as e:
        logger.exception(e)
        err = {"error": str(e)}
...
with gr.Blocks(title=f"验证码识别测试-{PLUGIN_NAME}") as demo:
    ...
    with gr.Tab("增强版"):
        ...
        icon_path_2_pro = os.path.join(CURRENT_PATH, "demo", "2_2.png")
        ...
        with gr.Row():
            result_output_pro = gr.JSON(label="识别结果")
