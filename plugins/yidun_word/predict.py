...
class YOLOV5_ONNX(object):
    def __init__(self, onnx_path, classes=None, providers=None):
        """初始化onnx"""
        if classes is None:
            classes = ['target']
        if not providers:
...
def detection(imgae):
    ...
    det_path = os.path.join(model_path, "Det.onnx")
    if not os.path.exists(det_path):
        raise CaptchaException(f"Error! 模型路径无效: '{det_path}'")
    imgDet = ImageDet(det_model_path=det_path)
    return imgDet.decect(imgae)
...
class OCR(object):
    def __init__(self, model_path, class_name_path):
        ...
        with open(class_name_path, "r", encoding="utf8") as f:
            self._class_name = json.loads(f.read())

    def predict(self, image_bytes: bytes, input_shape: tuple = (100, 100)) -> str:
        io_bytes = io.BytesIO(image_bytes)
        image = Image.open(io_bytes).convert("RGB").resize(input_shape)
        image_np = np.array(image, dtype="float32") / 255.0
        img_CHW = np.transpose(image_np, (2, 0, 1))
        image_np = np.expand_dims(img_CHW, 0)
        out = self._session.run(None, {"input.1": image_np})
        class_index = np.argmax(out, axis=-1)[0][0]
        return self._class_name[class_index]


class SiamesePredict(object):
    def __init__(self):
        model_path = os.path.join(os.path.dirname(__file__), 'model', 'Siamese.onnx')
        if not os.path.exists(model_path):
            raise CaptchaException(f"Error! 模型路径无效: '{model_path}'")
        self.onnx = onnxruntime.InferenceSession(model_path)

    @staticmethod
    def load_img(image_, input_shape: tuple = (105, 105)):
        if isinstance(image_, str):
            image = Image.open(image_).convert("RGB").resize(input_shape)
        elif isinstance(image_, bytes):
            io_bytes = io.BytesIO(image_)
            image = Image.open(io_bytes).convert("RGB").resize(input_shape)
        elif isinstance(image_, Image.Image):
            image = image_
        else:
            raise Exception("未知的图片类型")
        image_np = np.array(image, dtype="float32") / 255.0
        img_CHW = np.transpose(image_np, (2, 0, 1))
        image_np = np.expand_dims(img_CHW, 0)
        return image_np

    def predict(self, image_1, image_2) -> float:
        img1 = self.load_img(image_1)
        img2 = self.load_img(image_2)
        out = self.onnx.run(None, {"x1": img1, "x2": img2})
        try:
            return out[0][0][0]
        except:
            return 0.0
